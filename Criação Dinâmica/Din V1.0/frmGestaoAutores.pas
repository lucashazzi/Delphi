...
type
  ...
  private
    ...
    FAutor: TAutor;
    procedure CriarBtnExcluirLivros(Sender: TObject);
...
procedure TFormAutores.CriarBtnExcluirLivros(Sender: TObject);
begin
...
begin
  ...
  try
    ...
    raise Exception.Create
      ('[FormAutores - 340]: Erro ao se conectar com o Banco!');
  end;
  ...
  try
    ...
    begin
      FLivro := TLivroDAO.Carregar(_qry.FieldByName('IdLivro').Text.ToInteger());
      ...
      with TLabel.Create(ArrPanelAutores[_qry.RecNo]) do
      begin
      ...
      _btnPress := TButton.Create(ArrPanelAutores[_qry.RecNo]);
      _btnPress.Parent := ArrPanelAutores[_qry.RecNo];
      _btnPress.Caption := 'Excluir';
      _btnPress.Align := alRight;
      _btnPress.Width := 50;
      _btnPress.Height := 8;
      _btnPress.Tag := _qry.RecNo;
      _btnPress.OnClick := CriarBtnExcluirLivros;
